...
This proposition supplies the most direct exact bridge in the project. Every non-trivial zeta zero is a zero of a binary alternating Dirichlet series. The bridge is exact, but it remains many-channel: the alternating series contains infinitely many complex terms. The two-channel cancellation lemma cannot be applied without a canonical reduction.
...
\end{align}
Then $\eta=O-E$. It is tempting to interpret a zero of eta as equality of two channels. However, after analytic continuation, at a non-trivial zeta zero both $O$ and $E$ vanish because both contain the factor $\zeta(s)$. The equality is then $0=0$, not a normalized cancellation of two non-zero amplitudes.
...
\section{Alternation, phase, and $\pi$}

The sign $(-1)^{n-1}$ is $e^{i\pi(n-1)}$. Thus the eta series inserts a fixed phase increment $\pi$ between successive parity classes. This is the same elementary phase responsible for two-channel destructive interference and spinorial sign closure. Again, exact common ingredients do not automatically produce identical mechanisms:
\begin{itemize}
  ...
  \item at a zeta zero, global cancellation occurs across the entire analytically continued series.
\end{itemize}
A canonical coarse-graining would need to map the infinite alternating structure into a normalized pair while preserving zerohood and involution covariance.

\section{The eta value $\ln2$ and the half point}

The exact statements now form a chain of shared structures:
\[
 \eta(1)=\ln2=\Hbin(1/2),
\]
and
...
\]
The first equation relates the alternating harmonic series to balanced binary information. The second relates the alternating zeta function to non-trivial zeros. What is not established is an implication
\[
 \eta(s)=0\quad\Longrightarrow\quad\Hbin(\Re s)=\ln2.
\]
That implication is essentially RH expressed in informational language. The monograph does not hide it.

\section{A useful reformulation of the open bridge}

Instead of asking vaguely whether ``binary information explains RH'', one may ask:
...
\end{enumerate}
\end{problem}
If such a construction existed, \cref{lem:exact-cancellation} would force all eta zeros in the strip, hence all non-trivial zeta zeros, onto the half-axis. The difficulty of satisfying all four requirements is the precise content of the open programme.

\status{The eta identity, value at one, prefactor zeros, and zero equivalence in the strip are exact. The normalized binary reduction is open.}
