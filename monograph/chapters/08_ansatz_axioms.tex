...
The preceding chapters established a network of exact correspondences. To convert that network into a research programme, the assumptions must be stated in a form that can be tested, weakened, or rejected. The axioms below are not asserted as properties already possessed by the zeta function. They define the class of information-spinor models under consideration.
...
This axiom is the most direct identification of the analytic real part with binary balance. It is compatible with $\sigma\leftrightarrow1-\sigma$, but it is not forced by classical zeta theory. A stronger realization would derive these weights from a kernel or operator rather than assume them.
...
This postulate is SOH-C001 in explicit form. It remains unresolved for the information-spinor state-map branch.
...
\end{equation}
Because $P_+$ is positive semidefinite, this occurs exactly when $P_+\ket{\Psi(s)}=0$. The expression is gauge invariant and non-negative.

A candidate positive realization can therefore seek
\begin{equation}
 |\xi(s)|^2=w(s)\bra{\Psi(s)}P_+\ket{\Psi(s)},
 \qquad w(s)>0\quad(s\in\Sstrip).
 \label{eq:positive-factorization}
\end{equation}
This is a construction target, not a proved identity. It avoids phase-gauge ambiguity and preserves the zero set if the stated assumptions are met.
...
\end{enumerate}
A map achieving only the first level has little explanatory value. Reaching the later levels would close progressively stronger construction requirements, but none is assumed here.

\section{The role of the imaginary coordinate}

The axioms determine only the horizontal balance coordinate. The imaginary part $t$ must enter through phase, dynamics, or spectral quantization in any realization of this model.

\textbf{INTERPRETACJA.} Within the information-spinor model one may read $\sigma$ as a balance coordinate and $t$ as a spectral-phase or frequency coordinate. This is a modelling interpretation only; it is not an established physical ontology and it is not used as a premise in any theorem.

\status{The axioms define the information-spinor ansatz. Only the elementary consequences are proved. The zero-equivalence and canonical state-map construction requirements remain unfulfilled in this branch, even though later chapters construct other canonical quotient and kernel objects.}
