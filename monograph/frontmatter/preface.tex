...
coordinates, and the analytic geometry of the Riemann kernel.  Version 1.0
takes the shorter route: it begins from the proposed universal relational law
...
Accordingly the current theorem is
\[
\boxed{\mathrm{A0}\Longrightarrow\mathrm{RH}.}
\]

\paragraph{Historical-status note.}
Chapters 1--57 preserve the development history of the programme.  They
contain older OPEN labels, alternative RH-equivalent criteria, finite
certificates, and theorem-level no-go results.  Those labels should be read as
the proof state at the time of each route.  Version 1.0 does not delete that
record; instead Chapters 58--59 supersede it as the minimum proof graph.
