...
\status{SOH-G019 proved: a canonical positive-real-part disk, a classical Carath\'eodory-class normalization, and an exact normalized coefficient law.}
...
\section{Carath\'eodory normalization}

Define the unit-disk normalization
\[
 \boxed{
 H(\zeta):=\frac{F(R_\star\zeta)}{F(0)}.
 }
\]
Then $H$ is analytic on the unit disk, $H(0)=1$, and the preceding theorem gives
\[
 \boxed{\Re H(\zeta)>0\qquad(|\zeta|<1).}
\]
Therefore $H$ belongs to the classical Carath\'eodory class of analytic functions on the unit disk with positive real part and normalization $H(0)=1$. This is the precise reason for the terminology in the chapter title; no additional Carath\'eodory theorem is required for the zero-free conclusion proved above.
...
\]
The scaled positive-index coefficient sequence is therefore exactly normalized.
...
\[
 F(r)-F(0)\le F(0)
\]
becomes saturated. The resulting normalized sequence $(p_n)_{n\ge1}$ and Carath\'eodory normalization provide exact coefficient-level objects for subsequent PF and Jensen analysis.

\gap{Real-rootedness of $F$, PF$_3$, PF$_\infty$, and the SOH-G003 real-rootedness criterion remain open. No Riemann-Hypothesis conclusion is made in this chapter.}
