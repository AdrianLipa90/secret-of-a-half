...
def test_half_layer_maps_to_riemann_half() -> None:
    ...
    assert exact_x_to_t(Fraction(1, 2)) == Fraction(0)
    assert x_to_u(0.5) == pytest.approx(1.0)
    assert x_to_s(0.5) == pytest.approx(0.5)
    assert x_to_t(0.5) == pytest.approx(0.0)
...
def test_centered_coordinate_round_trips() -> None:
    for u in [0.125, 0.5, 1.0, 2.0, 32.0]:
        assert t_to_u(u_to_t(u)) == pytest.approx(u, rel=1e-14)
    for s in [0.1, 0.25, 0.5, 0.75, 0.9]:
        assert t_to_s(s_to_t(s)) == pytest.approx(s, rel=1e-14)


def test_fundamental_chain_is_exact_bloch_rapidity_orbit() -> None:
    chain = [Fraction(16), Fraction(8), Fraction(4), Fraction(2), Fraction(1), Fraction(1, 2)]
    mapped = [exact_x_to_t(x) for x in chain]
    expected = [
        Fraction(31, 33),
        Fraction(15, 17),
        Fraction(7, 9),
    ...
    assert mapped == expected
    for current, nxt in zip(mapped, mapped[1:]):
        assert exact_halving_t(current) == nxt


def test_halving_and_odd_branches_in_centered_coordinate() -> None:
    for n in range(1, 65):
        t = exact_x_to_t(Fraction(n))
        transformed = exact_halving_t(t) if n % 2 == 0 else exact_odd_collatz_t(t)
        expected = exact_x_to_t(Fraction(collatz_step_x(n)))
        assert transformed == expected
        floating = halving_t(float(t)) if n % 2 == 0 else odd_collatz_t(float(t))
        assert floating == pytest.approx(float(expected), abs=1e-14)


def test_single_halving_is_scale_half_hyperbolic_translation() -> None:
    for t in [Fraction(-1, 2), Fraction(0), Fraction(1, 3), Fraction(31, 33)]:
        assert exact_scale_t(t, Fraction(1, 2)) == exact_halving_t(t)
        assert scale_t(float(t), 0.5) == pytest.approx(float(exact_halving_t(t)), abs=1e-14)


def test_five_halvings_compose_to_minus_31_over_33_parameter() -> None:
    p = Fraction(-1, 3)
    total = p
    for _ in range(4):
        total = compose_boost_parameters(total, p)
    assert total == Fraction(-31, 33)

    t = Fraction(31, 33)
    for _ in range(5):
        t = exact_halving_t(t)
    assert t == 0
    assert exact_scale_t(Fraction(31, 33), Fraction(1, 32)) == 0
