...
def main() -> None:
    ...
    if len(includes) != 59:
        fail(f"v0.11 + RZ treatise integration requires exactly 59 numbered chapters, found {len(includes)}")
    if includes[-4:] != ["56_g024_complete_monotonicity_route_no_go", "57_reciprocal_conjugation_orbit_collapse", "58_occam_relational_zero_triad", "59_zero_critical_relational_axis"]:
        fail(f"unexpected publication terminal chapters: {includes[-4:]}")
    ...
    print(f"version=v0.11-publication-audit+rz-treatise chapters={len(includes)} canonical_through=SOH-G023 rz=integrated g024_t=reviewed_route_nogo")
