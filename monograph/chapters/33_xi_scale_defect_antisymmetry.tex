...
\section{Historical route boundary}

No nonconstant factor of automorphy
\[
X(au)=M_a(u)X(u)
\]
has been independently derived. Defining \(M_a\) as the quotient \(X(au)/X(u)\) would be tautological away from zeros. Historical route note: SOH-G008 alone did not provide zero location, and SOH-G003 was still open on this branch. The Version 1.1 proof does not rely on this route; it is completed independently in Chapters~58--59.
